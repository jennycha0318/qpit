...
- **결제(메시지당 과금):** 미구현 = 다음 레이어. 결정사항(결제사·가격·무료 한도) + 사용자의 결제계정/키 세팅 필요. ⚠️ 결제/금융 트랜잭션·크리덴셜 입력은 어시스턴트가 대신 못 함.
- **상대 파악:** ✅ '상대' 단계에 상대 자유서술 입력 추가 → AI(interpret)가 상대 성향(애착·관심도·주의 신호)을 추론해 해석·문구에 반영(점수 미반영). 다음 강화안 = **상대 메시지/캡처 해석**(가장 강력한 차별점, 아래 '받은 메시지 해석'과 연결).
