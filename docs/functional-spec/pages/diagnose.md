...
3. **survey** — 단계별 설문 (썸 9 / 연애 9 / 이별 후 11 문항)
   - 선택지 클릭 시 0.2초(200ms) 후 **자동 다음 문항** (연타는 `advancingRef`로 차단)
...
- **이별 후(breakup)**: 사귄 기간(`period`) · 헤어진 기간(`since`) · 이별 제안자(`who`) · 이별 이유(`reason`) · 현재 연락 상태(`contact`) · 새로운 사람(`newperson`) + 공통 4 = **11문항**
...
| 선택지 클릭 | 답 저장 + 200ms 후 자동 진행(마지막이면 진단) |
